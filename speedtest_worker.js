...
var settings = {
	...
	xhr_dlMultistream: 64, // number of download streams to use (can be different if enable_quirks is active)
	xhr_ulMultistream: 32, // number of upload streams to use (can be different if enable_quirks is active)
...
this.addEventListener("message", function(e) {
	...
	if (params[0] === "start" && testState === -1) {
		const node=JSON.parse(params[1])['node']
		if (JSON.parse(params[1])['node']=='xy') {
			let xhr=new XMLHttpRequest()
			xhr.open('GET','https://api-speedtest.onethingpcs.com/api/get_nodes?nodesnum=40',false)
			xhr.responseType='json'
			xhr.onload=e=>{
				let nodeinfo=(xhr.response).nodes
				let xy_nodes=[]
				for (let i = 0; i < (xhr.response).nodes.length; i++) {
				   xy_nodes.push(`${nodeinfo[i].scheme}://${nodeinfo[i].host}:${nodeinfo[i].port}/download?token=${xhr.response.token}&filesize=536870912&r=`)	
				}
				dlurl=xy_nodes
				emptyurl=dlurl
				console.log(dlurl);
...
function getIp(done) {
	...
	xhr.onload =  function() {
		...
		try {
			var data = JSON.parse(xhr.response)
				...
				clientIp = `${data.ip} ${data.country} ${data.org} `
				ip=data.ip
	...
	xhr.open("GET", 'https://ipinfo.io/json?token=c7b36fbc2b3fe7', true);
	xhr.send();
}
		...
		setTimeout(
			function() {
				...
				if (dlurl.constructor==Array) {
						...
						xhr[i].open("GET", dlurl[i] + Math.random(), true); // random string to prevent caching
				        xhr[i].send();
				
				}else{
				xhr[i].open("GET", dlurl + Math.random(), true); // random string to prevent caching
...
async function pingTest(done) {
	...
	xhr = [];
	//
	let dlip,dlcolo,_dlip,domain
	if (dlurl.constructor==Array) {
		 domain=dlurl[0].split(/:\/\//)[1]
	domain=domain.split(/\//)[0]
	const ipfilter=/\:/ig
			if (ipfilter.test(domain)) {
				domain=domain.split(':')[0]
			}
	}else{
		domain=dlurl.split(/:\/\//)[1]
	    domain=domain.split(/\//)[0]
	    const ipfilter=/\:/ig
			if (ipfilter.test(domain)) {
				console.log(domain);
			...
			let _colo
			let cf=/cloudflare/ig
			async function CheckCf() {
				let check
				const CheckIpv4=/((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3}/
				if (CheckIpv4.test(domain)) {
				await fetch(`https://ipinfo.io/${domain}/json?token=c7b36fbc2b3fe7`).then(async e=>{dlcolo=await e.json();
				check= cf.test(dlcolo.org)
			})
				return check

				}else{
				await fetch(`https://openai.dingding.wtf/resolve?name=${domain}&type=A&edns_client_subnet=${ip}`).then(async e=>{dlip=await e.json()})
				for (let i = 0; i < dlip.Answer.length; i++) {
					
					if (CheckIpv4.test(dlip.Answer[i].data)) {
						_dlip=dlip.Answer[i].data
						break
					}
					
				}
				
				await fetch(`https://ipinfo.io/${_dlip}/json?token=c7b36fbc2b3fe7`).then(async e=>{dlcolo=await e.json();
				check= cf.test(dlcolo.org)
			})
			return check
		}
			...
			if (await CheckCf()) {
				fetch(`https://${domain}/cdn-cgi/trace`).then(async e=>{
				_colo=(await e.text()).split('\n')[6].split('=')[1]
				colo=`测速服务器归属: ${_colo}`
			})
			}else{
				
			dlcolo=`${dlcolo.country} ${dlcolo.org}`
			colo=`测速服务器归属: ${dlcolo}`
			}
	// ping function
	var doPing = async function() {
		tverb("ping");
		pingProgress = i / settings.count_ping;
		prevT = new Date().getTime();
		xhr[0] = new XMLHttpRequest();
		xhr[0].onload = async function() {
			// pong
			tverb("pong");
			if (i === 0) {
